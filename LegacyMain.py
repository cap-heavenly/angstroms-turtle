...
def square_fill(whichtle, colour, angle, squaresize):
    whichtle.speed(0)
    whichtle.penup()
    whichtle.seth(0)
...
# Function to move the turtle by a certain amount squares
# +dy/-dy = up/down, +dx/-dx = left/right
def coord_move(dy, dx, squaresize):
    antle.penup()
    antle.seth(0)
...
def smiley(squaresize):
    square_fill(antle, "black", antle.heading(), squaresize)
    coord_move(-3, 0)
    square_fill(antle, "black", antle.heading(), squaresize)
    coord_move(0, -2)
    square_fill(antle, "black", antle.heading(), squaresize)
...
def heart(squaresize):
    square_fill(antle, "red", antle.heading(), squaresize)
    coord_move(0, -1)
    square_fill(antle, "red", antle.heading(), squaresize)
    coord_move(1, 0)
    square_fill(antle, "red", antle.heading(), squaresize)
...
def sebbycake(squaresize):
    for i in range(2):
        for i in range(5):
            coord_move(1, 0)
            square_fill(antle, "burlywood1", antle.heading(), squaresize)
        coord_move(-5, 1)
    for i in range(5):
        coord_move(1, 0)
        square_fill(antle, "DeepPink1", antle.heading(), squaresize)
    coord_move(-3, 1)
    square_fill(antle, "chartreuse1", antle.heading(), squaresize)
    coord_move(0, 1)
    square_fill(antle, "orange", antle.heading(), squaresize)
    coord_move(2, -1)
    square_fill(antle, "chartreuse1", antle.heading(), squaresize)
    coord_move(0, 1)
    square_fill(antle, "orange", antle.heading(), squaresize)
...
def the_ant(squaresize):
    for i in range(1000):
        antle.speed(1)
        antle.penup()
        if colour_check(squaresize) == "white":
            square_fill(antle, "black", antle.heading(), squaresize)
            antle.right(90)
            antle.forward(int(squaresize))
        elif colour_check(squaresize) == "black":
            square_fill(antle, "white", antle.heading(), squaresize)
            antle.left(90)
            antle.forward(int(squaresize))
